...
const FORBIDDEN_PATTERNS = [
  ...
  // 2026-07-10 운영자 지시: AI 말버릇 차단 — 긴 줄표(em/en dash)는 AI 티가 나는 대표 습관.
  // AdSense 재승인 관점에서 "AI로 찍어낸 사이트" 인상 방지. 신규 글부터 적용(기존 발행글 불변).
  // 대체: 쉼표(,) · 중점(·) · 콜론(:) · 물결(~). U+2014(—) / U+2013(–) 모두 금지.
  { pattern: /[—–]/g, label: '긴 줄표(em-dash, AI 말버릇)' },
  // 2026-07-22 운영자 지시: 픽토그램 이모지도 동일 취지로 금지 (⚠️→"주의:" 텍스트, UI는 Icon.tsx).
  // 신규 글부터 적용(기존 발행글 불변). 화살표(→)·체크(✓ U+2713)·엑스(✗ U+2717)는
  // 표 안 수치·비교 표기 용도로 게이트에서 제외 (범위에서 의도적으로 뺌).
  {
    pattern: /[\u{1F000}-\u{1FAFF}\u{2600}-\u{26FF}\u{2700}-\u{2712}\u{2714}-\u{2716}\u{2718}-\u{27BF}]/gu,
    label: '이모지·픽토그램(AI 말버릇)',
  },
];
