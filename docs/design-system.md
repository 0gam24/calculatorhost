...
## 6. 광고 (Google Auto ads 단독 운영)

> **2026-08-12 운영자 결정**: 손으로 배치하던 수동 슬롯(AdSlot·InfeedAd·SkyscraperAd·MobileAnchorAd)을 **전면 폐지**하고 Google Auto ads 단독 운영으로 전환. 고정 광고 박스가 페이지마다 박혀 있으면 프리미엄 인상을 해치고, 배치 최적화는 Auto ads 쪽이 더 낫다는 판단.

- 광고 삽입 위치·개수·포맷은 **Google Auto ads 가 결정**한다. 페이지 코드에 광고 컴포넌트를 두지 않는다.
- 유일한 통합 지점: `src/app/layout.tsx` 의 `adsbygoogle.js` 스크립트 (게시자 ID는 환경변수).
- 밀도·라벨·다크 배경 등 표시 규정도 Auto ads 가 자체 처리한다. 우리 쪽 CSS 로 광고를 재정의하지 않는다.
- 회귀 가드: `tests/e2e/adsense.e2e.ts` 가 (1) Auto ads 스크립트 존재 (2) 수동 슬롯 마크업 부재 를 검증.
- 되돌리려면 커밋 이력에서 `src/components/ads/` 복원.
...
## 9. 레이아웃 프레임 (2026-08-12 Auto ads 전환 반영)
```
┌──────┬──────────────────────────────────────┐
│      │ [헤더 (로고·검색·테마토글)]            │
│      ├──────────────────────────────────────┤
...
광고는 위 구조 위에 Google Auto ads 가 알아서 삽입한다.
고정 슬롯 자리를 코드로 예약하지 않는다.
```

### 광고 배치 (2026-08-12~ Auto ads 단독)

수동 슬롯 5종(AD-1~AD-5) 아키텍처는 폐지됐다. 배치는 Google Auto ads 가 결정하므로
본 문서는 **광고가 들어올 수 있는 자리를 비워 두는 원칙**만 규정한다.

| 원칙 | 내용 |
|---|---|
| 본문 우선 | 콘텐츠 컬럼(`max-w-3xl`)의 가독성이 광고보다 우선한다 |
| 여백 확보 | 섹션 사이 여백(`space-y-8`)이 Auto ads 삽입 지점이 된다 |
| CLS | 고정 높이 예약은 Auto ads 가 처리. 우리 쪽에서 빈 자리를 만들지 않는다 |
| 금지 | 광고를 흉내 낸 UI, 광고 옆 오인 유도 배치, 광고 위장 내비게이션 |
...
- 2026-08-12: 수동 광고 슬롯 5종 아키텍처 폐지 → Google Auto ads 단독 운영으로 전환 (운영자 결정). §6 재작성, §9 레이아웃 프레임에서 광고 자리 제거.
