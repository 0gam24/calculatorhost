...
export default function RealEstateCategoryPage() {
  ...
  const webPageLd = buildWebPageJsonLd({
    ...
    url: 'https://calculatorhost.com/category/real-estate/',
            ...
            <article className="mx-auto max-w-4xl space-y-8">
              ...
              <section className="space-y-6">
                ...
                <div className="grid gap-6 sm:grid-cols-2">
                  {CALCULATORS.map((calc) => (
                    ...
                    >
                      ...
                      <Link
                        href={calc.href}
                        className="inline-flex w-fit items-center gap-1.5 rounded-lg bg-danger-500 px-4 py-2 font-semibold text-white transition-colors hover:bg-danger-600"
                      >
                ...
                {(() => {
                  ...
                  const additional = CATEGORY_CALCULATORS['real-estate'].filter((s) => !cardSlugs.has(s));
                  if (additional.length === 0) return null;
                  return (
                    <div className="mt-6 rounded-lg border border-border-base bg-bg-card p-5">
                      <h3 className="mb-3 text-lg font-semibold text-text-primary">
                        추가 부동산 계산기 ({additional.length}개)
                      </h3>
              ...
              {(() => {
                const labels = GUIDE_CATEGORY_LABEL['real-estate'];
                const cross = new Set(CROSS_GUIDES['real-estate']);
                ...
                return (
                  <section aria-label="관련 가이드" className="space-y-4 rounded-lg border border-border-base bg-bg-card p-6">
                    ...
                    <p className="text-sm text-text-secondary">
                      부동산 매매·임대·세제 관련 실전 가이드. AI Overview 인용 가능한 §N 법조항 기반.
                    </p>
                    <ul className="grid gap-2 sm:grid-cols-2">
                      {related.map((g) => (
                        <li key={g.slug}>
              ...
              {(() => {
                const ref = CATEGORY_GLOSSARY['real-estate'];
                if (!ref) return null;
                return (
