...
export default function LifestyleCategoryPage() {
  ...
  const webPageLd = buildWebPageJsonLd({
    ...
    url: 'https://calculatorhost.com/category/lifestyle/',
            ...
            <article className="mx-auto max-w-4xl space-y-8">
              ...
              <section className="space-y-6">
                ...
                {(() => {
                  ...
                  const additional = CATEGORY_CALCULATORS.lifestyle.filter((s) => !cardSlugs.has(s));
                  if (additional.length === 0) return null;
                  return (
                    <div className="mt-6 rounded-lg border border-border-base bg-bg-card p-5">
                      <h3 className="mb-3 text-lg font-semibold text-text-primary">
                        추가 생활 계산기 ({additional.length}개)
                      </h3>
              ...
              {(() => {
                const labels = GUIDE_CATEGORY_LABEL.lifestyle;
                const cross = new Set(CROSS_GUIDES.lifestyle);
              ...
              {(() => {
                const ref = CATEGORY_GLOSSARY.lifestyle;
                if (!ref) return null;
                return (
