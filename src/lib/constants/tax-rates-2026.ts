...
/**
 * 연납(1월 일괄) 할인율 — 지방세법 시행령 §125
 *
 * 검증 완료 (2026-06-02):
 * - 현행: 5% (2025.1.1 시행, 2026 유지)
 * - 법적근거: 지방세법 시행령 제125조 (자동차세 연세액 일시납부 및 세액공제)
 * - 적용방식: 연간 총액(자동차세 + 지방교육세) × (선납일수/365) × 5%
 * - 산정식: 공제액 = 연세액 × (선납일수 / 365) × 0.05
 * - 선납일수: 납부기한 다음날 ~ 12월 31일 (1월 신청 기준 약 349~351일)
 * - 실효율(1월 신청): (351/365) × 5% ≈ 4.58%
 * - 실효율(3월 신청): (273/365) × 5% ≈ 3.74%
 * - 실효율(6월 신청): (214/365) × 5% ≈ 2.51%
 * - 실효율(9월 신청): (122/365) × 5% ≈ 1.25%
 *
 * 출처:
 * - 경향신문 2025-01-12 "자동차세 연납할인율 5% 유지"
 * - 서초구청·성동구청 2025년 자동차세 기간납부 안내
 * - 국세청 자동차세 납부기간 공지
 *
 * @see ADR-014-vehicle-annual-discount-rate-fixed
 */
export const VEHICLE_TAX_ANNUAL_PAYMENT_DISCOUNT_RATE = 0.05;
