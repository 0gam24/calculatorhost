...
- [ ] **각 H2 섹션마다 "다만," 또는 "예외:" 문단 1개+** (각 섹션 말미 — 2026-07-22부터 ⚠️ 등 이모지 라벨 금지, 텍스트 라벨만. §2-14 참조)
...
- [ ] **이모지·픽토그램 전면 금지 (2026-07-22 운영자 지시 확장)**: 신규 콘텐츠의 제목·본문·표·FAQ·메타 어디에도 이모지(💡✅❌⚠️📌🏠💰 등 픽토그램) 사용 금지. 이모지는 "AI 자동생성 사이트" 인상의 대표 신호.
  - 주의·예외 표기: `⚠️` → "주의:" / "다만," 텍스트 라벨
  - 체크·불릿: `✅❌` → 표(가능/불가) 또는 "가능·불가" 텍스트
  - UI 크롬(카드·버튼·목록 아이콘)은 `src/components/ui/Icon.tsx` SVG 라인 아이콘만 사용 (이모지 금지)
  - 본문 산문의 `A → B` 화살표 체인도 지양 — "A에서 B로" 자연문 권장 (표 안 수치 변화 표기는 허용)
  - 자동 게이트: `scripts/check-guide-quality.mjs` FORBIDDEN_PATTERNS 가 픽토그램 이모지 red 처리
- [ ] content-writer 위임 프롬프트에 본 §2-14 명시 (§2 일괄 인용 시 자동 포함)

> **2026-08-12 소급 정리 완료 + 제목 방침 변경**: 운영자 지시로 전 사이트를 소급 정리했다.
> 렌더링 이모지 30파일 제거, 긴 줄표 351파일 1,616라인 치환, **제목 291개도 문장부호만 치환**.
> 따라서 "기존 발행글 제목 불변" 규칙은 이제 **단어·키워드를 바꾸는 리타이틀에만** 적용된다.
> 키워드가 그대로인 문장부호 정규화(— → , / · / :)는 허용.
